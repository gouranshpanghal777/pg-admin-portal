...
import { admitTenant, createStaffAccount, deactivateStaffAccount, deleteCashbookEntryCascade, deleteTenantWithPayments, loadAppData, persistAppData, recordSplitPayment, undoVacateTenant, updateUnsettledTenantRent, vacateTenantErp } from './lib/database'
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f7f3ec] text-slate-900">
      ...
      {modal === 'cashbook' && <CashbookModal branches={data.branches.filter((item) => item.active !== false)} branchId={branchId} categories={Array.from(new Set(scoped.cashbook.map((entry) => entry.category).filter((value): value is string => Boolean(value))))} onClose={() => setModal('')} onSubmit={(entry) => { const interBranch = parseInterBranchReference(entry.reference); updateData((previous) => { const primary = { id: uid('c'), branchId, source: 'Manual' as const, ...entry }; const mirror = interBranch?.kind === 'IBS' ? { id: uid('c'), branchId: interBranch.counterpartyBranchId, source: 'Manual' as const, type: 'Debit' as const, amount: interBranch.amount, description: `Inter-branch settlement paid to ${branch.name}`, date: entry.date, category: 'Inter-branch Settlement', paymentMode: entry.paymentMode, reference: `IBS|${branchId}|${interBranch.amount}`, remarks: entry.remarks } : undefined; return { ...previous, cashbook: [primary, ...(mirror ? [mirror] : []), ...previous.cashbook] } }, `Add ${entry.type}`, 'Cashbook', `${role} ${currentUser.name} added cashbook ${entry.type.toLowerCase()} of ${money(entry.amount)}. Description: ${entry.description}.`, { amount: entry.amount, type: entry.type }) }} />}
      {modal === 'editCashbook' && <CashbookModal entry={data.cashbook.find((entry) => entry.id === selectedCashbookId)} branches={data.branches.filter((item) => item.active !== false)} branchId={branchId} categories={Array.from(new Set(scoped.cashbook.map((entry) => entry.category).filter((value): value is string => Boolean(value))))} onClose={() => setModal('')} onSubmit={(entry) => { const old = data.cashbook.find((item) => item.id === selectedCashbookId)!; updateData((previous) => ({ ...previous, cashbook: previous.cashbook.map((item) => item.id === selectedCashbookId ? { ...item, ...entry } : item) }), 'Edit Entry', 'Cashbook', `${role} ${currentUser.name} edited cashbook entry ${old.description}. Amount changed from ${money(old.amount)} to ${money(entry.amount)}; type ${old.type} to ${entry.type}.`) }} />}
      {modal === 'admit' && <AdmitTenantModal rooms={scoped.rooms} tenants={scoped.activeTenants} canReceivePayment={can('add_payment')} onClose={() => setModal('')} onSubmit={async (requestId, paymentRequestId, tenant, initialPayment) => {
        setBackendError('')
        try {
      ...
      {modal === 'vacate' && (() => { const tenant = data.tenants.find((item) => item.id === selectedTenantId)!; const rentState = scoped.rentStates.get(tenant.id) || rentLedgerState(tenant, scoped.obligations, scoped.payments); return <VacateModal tenant={tenant} dueDate={rentState.dueDate} onClose={() => setModal('')} onSubmit={async (left, settlementRequestId) => { setBackendError(''); try { if ((left.settlementReceived || 0) > 0) await recordSplitPayment({ requestId: settlementRequestId, tenantId: tenant.id, branchId, rentAmount: left.settlementReceived || 0, securityAmount: 0, electricityAmount: 0, otherAmount: 0, paymentDate: left.leftDate, rentPeriod: rentState.period, paymentMode: 'Cash', description: `Vacate settlement - ${tenant.name}` }); await vacateTenantErp(selectedTenantId, left); setData(await loadAppData()) } catch (error) { setBackendError(error instanceof Error ? error.message : 'Unable to vacate tenant'); throw error } }} /> })()}
      {modal === 'confirmUndoVacate' && <ConfirmModal title="Undo tenant vacate" message="Restore this tenant to the original room and reverse any security refund or deduction created during vacating? Payment history will remain unchanged." confirmLabel="Undo Vacate" onClose={() => setModal('')} onConfirm={async () => { setBackendError(''); try { await undoVacateTenant(selectedTenantId); setData(await loadAppData()) } catch (error) { const message = error instanceof Error ? error.message : 'Unable to undo tenant vacate'; setBackendError(message); throw error } }} />}
      {modal === 'editTenant' && <EditTenantModal tenant={data.tenants.find((tenant) => tenant.id === selectedTenantId)!} rooms={scoped.rooms} tenants={scoped.activeTenants} onClose={() => setModal('')} onSubmit={(changes) => { const tenant = data.tenants.find((item) => item.id === selectedTenantId)!; updateData((previous) => ({ ...previous, tenants: previous.tenants.map((item) => item.id === selectedTenantId ? { ...item, ...changes } : item) }), 'Edit Tenant', 'Tenants', `${role} ${currentUser.name} edited tenant ${tenant.name} details. Changed room from ${data.rooms.find((room) => room.id === tenant.roomId)?.number} to ${data.rooms.find((room) => room.id === changes.roomId)?.number}, rent from ${money(tenant.monthlyRent)} to ${money(changes.monthlyRent || 0)}.`); if (changes.monthlyRent !== undefined && changes.monthlyRent !== tenant.monthlyRent) void updateUnsettledTenantRent(tenant.id, changes.monthlyRent).catch((error) => setBackendError(error instanceof Error ? error.message : 'Unable to update rent obligations')) }} />}
      ...
      {modal === 'expense' && <ExpenseModal onClose={() => setModal('')} onSubmit={(expense) => { const expenseId = uid('e'); const cashbookId = uid('c'); updateData((previous) => ({ ...previous, expenses: [{ id: expenseId, branchId, cashbookId, ...expense }, ...previous.expenses], cashbook: [{ id: cashbookId, branchId, type: 'Debit', amount: expense.amount, description: expense.description, date: expense.date, source: 'Expense', linkedId: expenseId }, ...previous.cashbook] }), 'Add Expense', 'Finance', `${role} ${currentUser.name} added expense of ${money(expense.amount)} under ${expense.category}. Vendor/Note: ${expense.vendor || '-'}.`) }} />}
      ...
      {modal === 'confirmDeleteTenant' && <ConfirmModal title="Permanently delete tenant" message="This action will delete all linked records including payments, ledger entries and logs." confirmLabel="Delete Permanently" onClose={() => setModal('')} onConfirm={async () => {
        setBackendError('')
        try { await deleteTenantWithPayments(selectedTenantId); setData(await loadAppData()) }
      ...
      {modal === 'confirmDeleteCashbook' && <ConfirmModal title="Delete cashbook entry" message="This also deletes the linked payment, expense, maintenance cost, or inventory purchase and recalculates affected balances." onClose={() => setModal('')} onConfirm={async () => {
        setBackendError('')
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await onSubmit(
        requestId,
        paymentRequestId,
        { name: String(form.get('name')), phone: String(form.get('phone')), email: String(form.get('email')), roomId, bedNo: nextBed, joiningDate: String(form.get('joiningDate')), dueDate: String(form.get('dueDate') || form.get('joiningDate')), monthlyRent: Number(form.get('monthlyRent')), security: Number(form.get('security')), electricity: String(form.get('electricity')) as 'Included' | 'Fixed', electricityAmount: Number(form.get('electricityAmount') || 0), idProof: String(form.get('idProof') || '').replace(/\s/g, '') },
...
function VacateModal({ tenant, dueDate, onClose, onSubmit }: { tenant: Tenant; dueDate: string; onClose: () => void; onSubmit: (left: NonNullable<Tenant['left']>, settlementRequestId: string) => Promise<void> }) {
