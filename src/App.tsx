...
type SplitPaymentInput = { requestId: string; tenantId: string; rentAmount: number; securityAmount: number; electricityAmount: number; otherAmount: number; paymentDate: string; paymentMode: string; description: string }
...
function PaymentModal({ tenants, payments, obligations, selectedTenantId, onClose, onSubmit }: { tenants: Tenant[]; payments: Payment[]; obligations: PaymentObligation[]; selectedTenantId: string; onClose: () => void; onSubmit: (payment: SplitPaymentInput) => Promise<void> }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true); setError('')
    try {
      await onSubmit({ requestId, tenantId, rentAmount, securityAmount, electricityAmount, otherAmount, paymentDate: String(form.get('paymentDate')), paymentMode, description: String(form.get('description') || '') })
      onClose()
    } catch (failure) {
