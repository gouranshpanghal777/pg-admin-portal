...
export type LedgerPartyType = 'Staff' | 'Vendor' | 'Building Rent' | 'Other'
...
function App() {
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((event, next) => {
      if (event === 'SIGNED_OUT') {
        Object.keys(sessionStorage).filter((key) => key.startsWith('pg95-login:')).forEach((key) => sessionStorage.removeItem(key))
        try { purgePaymentDrafts(localStorage) } catch { /* storage unavailable */ }
      }
  ...
  const nav = ([
    ...
    { label: 'Staff Salary', icon: <IndianRupee size={18} /> },
  ...
  ] as { label: Page | 'Switch Branch' | 'Sign Out' | 'Staff Salary'; icon: ReactNode }[]).filter((item) => (isAdmin || item.label !== 'Settings') && (item.label !== 'Reports' || can('view_reports')))
  const handleNav = (label: Page | 'Switch Branch' | 'Sign Out' | 'Staff Salary') => {
    setMobileNav(false)
    if (label === 'Sign Out') {
      const previous = dataRef.current
    ...
    if (label === 'Staff Salary') { setPage('Finance'); setFinanceTab('Staff Salary'); return }
    setPage(label)
  }
  ...
  return (
    <div className="min-h-screen bg-[#f7f3ec] text-slate-900">
      ...
      {(modal === 'categoryAccountEntry' || modal === 'staffSalaryEntry') && <CategoryAccountEntryModal branchName={branch.name} categories={data.categories.filter((category) => category.branchId === branchId)} parties={modal === 'staffSalaryEntry' ? scoped.ledgerParties.filter((party) => party.type === 'Staff') : scoped.ledgerParties} entries={scoped.ledgerEntries} isAdmin={isAdmin} canAddCashbook={can('add_cashbook')} canAddExpense={can('add_expense')} onClose={closeModal} onSaved={async (message) => { const refreshed = await loadAppData(); dataRef.current = refreshed; setData(refreshed); setSuccessMessage(message) }} />}
      ...
      {modal === 'payment' && <PaymentModal key={`${currentUser.id}:${branchId}:${selectedTenantId}`} userId={currentUser.id} branchId={branchId} tenants={scoped.activeTenants} payments={scoped.payments} obligations={scoped.obligations} advances={scoped.advances} selectedTenantId={selectedTenantId} onClose={closeModal} onSubmit={async (payment) => {
        setBackendError('')
        try {
...
function Dashboard({ scoped, rentSummary, refreshRentSummary, setModal, setPage, setTenantTab, setTenantFilter, setRoomFloor, setFinanceTab, setPaymentFilter, setTicketFilter }: { scoped: ReturnType<typeof branchData>; rentSummary: RentCollectionSummary | null; refreshRentSummary: () => void; setModal: (value: string) => void; setPage: (page: Page) => void; setTenantTab: (value: 'Active' | 'Left PG') => void; setTenantFilter: (value: string) => void; setRoomFloor: (value: string) => void; setFinanceTab: (value: string) => void; setPaymentFilter: (value: string) => void; setTicketFilter: (value: string) => void }) {
  const canonicalSummary = {
    expectedTillMonthEnd: [...scoped.rentStates.values()].reduce((sum, state) => sum + state.expectedTillMonthEnd, 0),
    pendingTillToday: [...scoped.rentStates.values()].reduce((sum, state) => sum + state.pending, 0),
    previousMonthsPending: [...scoped.rentStates.values()].reduce((sum, state) => sum + state.previousRentDue, 0),
    tenantCountWithPending: [...scoped.rentStates.values()].filter((state) => state.expectedTillMonthEnd > 0).length,
  }
  const summaryMismatch = rentSummary && Math.abs(rentSummary.pendingTillToday - canonicalSummary.pendingTillToday) > 0.01
  const todayCollection = scoped.payments.filter((payment) => payment.date === today).reduce((sum, payment) => sum + payment.amount, 0)
  const monthlyCollection = scoped.payments.filter((payment) => payment.date.startsWith(currentMonth)).reduce((sum, payment) => sum + payment.amount, 0)
        ...
        <div className="relative overflow-hidden rounded-lg border-2 border-blue-200 bg-blue-50 p-4 shadow-sm">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-3xl font-black text-blue-900">{money(canonicalSummary.expectedTillMonthEnd || 0)}</p>
          ...
          <p className="mt-2 text-xs text-blue-500" title="Total outstanding rent for all periods up to and including the current month. Includes unpaid balances from previous months and full current month rent regardless of due date.">{canonicalSummary.tenantCountWithPending || 0} tenant{(canonicalSummary.tenantCountWithPending || 0) !== 1 ? 's' : ''} with pending balance</p>
        </div>
        <div className="relative overflow-hidden rounded-lg border-2 border-rose-200 bg-rose-50 p-4 shadow-sm">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p title={summaryMismatch ? 'Server aggregate differs from tenant balances; refresh and review before collection.' : undefined} className="mt-1 text-3xl font-black text-rose-900">{money(canonicalSummary.pendingTillToday || 0)}</p>
          ...
          <p className="mt-2 text-xs text-rose-500" title="Rent that has actually become due up to today. Includes all previous unpaid months and current month obligations whose due date has arrived.">{canonicalSummary.previousMonthsPending ? `Previous: ${money(canonicalSummary.previousMonthsPending)}` : 'No previous pending'}</p>
        </div>
      </div>
...
function BillCreator({ scoped, data, branch, setSelectedTenantId }: { scoped: ReturnType<typeof branchData>; data: AppData; branch: Branch; setSelectedTenantId: (value: string) => void }) {
  ...
  if (!tenant) return <Card>No tenant available for invoice generation.</Card>
  const account = scoped.rentStates.get(tenant.id) || getRentLedgerState(tenant, scoped.payments, scoped.obligations, scoped.advances)
  const lines = [
    ...account.rentLines.filter((line) => line.due && line.pending > 0).map((line) => ({ label: `Rent balance · ${formatMonth(line.period)}`, amount: line.pending })),
    { label: 'Electricity due', amount: account.electricityDue },
    { label: 'Security balance', amount: account.securityDue },
    { label: 'Other explicit charges', amount: account.otherDue },
  ].filter((line) => line.amount > 0)
  return <div className="invoice-layout grid gap-4 lg:grid-cols-[320px_1fr]">
    <Card className="no-print"><Field label="Select tenant"><select value={tenantId} onChange={(event) => { setTenantId(event.target.value); setSelectedTenantId(event.target.value) }} className={inputClass}>{scoped.activeTenants.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field></Card>
    <article className="invoice-sheet print-surface rounded-lg border border-slate-300 bg-white p-6 shadow-sm">
      <header className="border-b-2 border-slate-900 pb-5"><h2 className="text-2xl font-black">{branch.name}</h2><p>{branch.address}</p><h3 className="mt-3 font-bold">CURRENT DUES STATEMENT</h3><p className="text-xs text-slate-500">As of {formatDate(account.asOfDate)} · Not a replacement for historical invoices</p></header>
      <section className="border-b py-5 text-sm"><p className="text-lg font-bold">{tenant.name}</p><p>Room {room?.number || 'Archived room'}</p><p>{tenant.phone}</p>{account.pending > 0 && <p>Oldest unpaid rent due: {formatDate(account.dueDate)}</p>}</section>
      <table className="my-5 w-full text-sm"><thead><tr className="text-left"><th className="p-3">Outstanding charge</th><th className="p-3 text-right">Amount</th></tr></thead><tbody>{lines.map((line) => <tr key={line.label} className="border-b"><td className="p-3">{line.label}</td><td className="p-3 text-right">{money(line.amount)}</td></tr>)}</tbody></table>
      <p className="flex justify-between border-t-2 border-slate-900 pt-3 text-lg"><span>Total payable</span><b>{money(account.totalPayable)}</b></p>
      <p className="mt-2 text-xs text-slate-500">Amounts above are balances after recorded payments and applied advances; payments are not subtracted twice.</p>
      {account.credit > 0 && <p className="mt-3">Unapplied advance: {money(account.credit)} — confirm allocation separately.</p>}
      {tenant.electricity === 'Included' && <p className="mt-3 text-sm">Electricity included; no separate charge.</p>}
      {account.electricityNeedsReview && <p className="mt-3 text-sm text-amber-800">Fixed electricity is not billed for this period. Confirm separately; no historical charge has been added.</p>}
      <div className="no-print mt-6 flex justify-end"><Button tone="blue" onClick={() => window.print()}><Printer size={16} /> Print / Save PDF</Button></div>
    </article>
  </div>
}
...
function CashbookModal({ entry, categories, branches, branchId, allowAdvanced = false, onClose, onSubmit }: { entry?: CashbookEntry; categories: string[]; branches: Branch[]; branchId: string; allowAdvanced?: boolean; onClose: () => void; onSubmit: (entry: CashbookFormEntry) => Promise<void> | void }) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await onSubmit({
        ...
        rentBalance,
        rentDueDate,
        adjustRentLedger: adjustment && (balanceChanged || dueDateChanged),
        applyRentToPending: false,
      })
      onClose()
    } catch (failure) {
  ...
  return <Modal title="Edit Tenant" onClose={onClose}><form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
    ...
    <div className="md:col-span-2 grid gap-3 rounded-md border border-amber-200 bg-amber-50 p-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Field label="Selected period balance"><input disabled={!adjustment} className={inputClass} type="text" inputMode="decimal" pattern="[0-9]*[.]?[0-9]{0,2}" value={rentBalanceInput} onFocus={(event) => event.currentTarget.select()} onWheel={(event) => event.currentTarget.blur()} onChange={(event) => { const next = event.target.value; if (next === '' || /^\d*\.?\d{0,2}$/.test(next)) setRentBalanceInput(next.replace(/^0+(?=\d)/, '')) }} required /></Field>
        <Field label="Selected period due date"><input disabled={!adjustment} className={inputClass} type="date" value={rentDueDate} onChange={(event) => setRentDueDate(event.target.value)} required /></Field>
      </div>
...
function PaymentModal({ userId, branchId, tenants, payments, obligations, advances, selectedTenantId, onClose, onSubmit }: { userId: string; branchId: string; tenants: Tenant[]; payments: Payment[]; obligations: PaymentObligation[]; advances: AdvanceMovement[]; selectedTenantId: string; onClose: () => void; onSubmit: (payment: SplitPaymentInput) => Promise<void> }) {
  const draft = usePaymentDraft(paymentDraftKey(userId, branchId, selectedTenantId), () => ({
    requestId: crypto.randomUUID(), tenantId: selectedTenantId || tenants[0]?.id || '', paymentMode: 'Cash',
  ...
  const closeDraft = () => { if (window.confirm('Close this form and keep the payment draft for later?')) onClose() }
  const [saving, setSaving] = useState(false)
  const savingRef = useRef(false)
  const [error, setError] = useState('')
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    if (savingRef.current) return
    if (!tenant) { setError('Select an available tenant before submitting.'); return }
    ...
    savingRef.current = true; setSaving(true); setError('')
    try {
      const rentPeriod = draft.value.attempted ? draft.value.rentPeriod : rentState?.period
      draft.patch({ attempted: true, rentPeriod })
  ...
  return <Modal title="Add Received Payment" onClose={closeDraft}><form className="grid gap-4" onSubmit={submit}><fieldset disabled={saving || draft.value.attempted} className="grid gap-4"><Field label="Tenant"><select className={inputClass} value={tenantId} onChange={(event) => selectTenant(event.target.value)}>{tenants.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field><div className="grid grid-cols-2 gap-2 rounded-md bg-slate-50 p-3 text-sm"><p>Total due rent<br /><b>{money(rentBalance)}</b></p><p>Rent due date<br /><b>{formatDate(rentState?.dueDate)}</b></p><p>Security balance<br /><b>{money(securityBalance)}</b></p><p>Security received<br /><b className="text-emerald-700">{money(securityReceived)}</b></p><p>Security total<br /><b>{money(tenant?.security || 0)}</b></p><p>Electricity due<br /><b>{money(rentState?.electricityDue || 0)}</b></p><p>Other charges<br /><b>{money(rentState?.otherDue || 0)}</b></p><p>Total payable<br /><b>{money(rentState?.totalPayable || 0)}</b></p></div>{isFirstTimeSecurity && <p className="text-xs text-amber-700 bg-amber-50 rounded-md px-3 py-2">This tenant has no security deposit on record. The amount you enter will set their total security deposit.</p>}<div className="grid gap-4 sm:grid-cols-2"><Field label="Rent received"><input className={inputClass} type="number" min="0" step="0.01" inputMode="decimal" value={rentAmount || ''} placeholder="0" onWheel={(event) => event.currentTarget.blur()} onChange={(event) => draft.patch({ rentAmount: Number(event.target.value) })} /></Field><Field label={`Security deposit received${isFirstTimeSecurity ? ' (first-time)' : ''}`}><input className={inputClass} type="number" min="0" step="0.01" inputMode="decimal" max={isFirstTimeSecurity ? undefined : securityBalance} value={securityAmount || ''} placeholder={isFirstTimeSecurity ? 'Enter total security amount' : '0'} onWheel={(event) => event.currentTarget.blur()} onChange={(event) => draft.patch({ securityAmount: Number(event.target.value) })} /></Field><Field label="Electricity received"><input className={inputClass} type="number" min="0" step="0.01" inputMode="decimal" value={electricityAmount || ''} placeholder="0" onWheel={(event) => event.currentTarget.blur()} onChange={(event) => draft.patch({ electricityAmount: Number(event.target.value) })} /></Field><Field label="Other received"><input className={inputClass} type="number" min="0" step="0.01" inputMode="decimal" value={otherAmount || ''} placeholder="0" onWheel={(event) => event.currentTarget.blur()} onChange={(event) => draft.patch({ otherAmount: Number(event.target.value) })} /></Field></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Payment date"><input name="paymentDate" className={inputClass} type="date" value={paymentDate} onChange={(event) => draft.patch({ paymentDate: event.target.value })} required /></Field><Field label="Rent month being settled"><input className={inputClass} value={rentState?.period ? formatMonth(rentState.period) : '-'} disabled /></Field><Field label="Payment mode"><select className={inputClass} value={paymentMode} onChange={(event) => draft.patch({ paymentMode: event.target.value })}><option>Cash</option><option>UPI</option><option>Bank Transfer</option><option>Card</option></select></Field></div><Field label="Description/source optional"><input name="description" className={inputClass} maxLength={2000} value={draft.value.description} onChange={(event) => draft.patch({ description: event.target.value })} /></Field></fieldset><p className="text-xs text-slate-500">Payment draft stays on this device for 24 hours. Reopen this payment form to restore it. Do not enter identity or payment credentials in the description.</p>{draft.storageError && <p role="alert">{draft.storageError}</p>}{draft.value.attempted && <p role="status" className="text-sm text-amber-800">Submission values are locked for safe retry with the same request ID. Do not create another payment while this result is unresolved.</p>}{error && <p className="rounded-md bg-rose-50 p-3 text-sm text-rose-700">{error}</p>}<div className="flex justify-end gap-2"><Button tone="soft" onClick={closeDraft}>Keep draft & close</Button>{!draft.value.attempted && <Button tone="soft" onClick={() => { if (window.confirm('Discard this payment draft?')) { draft.clear(); onClose() } }}>Discard</Button>}<Button tone="green" type="submit" disabled={saving}>{saving ? 'Saving...' : 'Add Payment'}</Button></div></form></Modal>
}
...
function ManageCategoriesModal({ branchCategories, data, branch, updateData, role, currentUser, onClose }: { branchCategories: Category[]; data: AppData; branch: Branch; updateData: (updater: (previous: AppData) => AppData, action: string, entity: string, description?: string, metadata?: Record<string, string | number>) => void; role: Role; currentUser: User; onClose: () => void }) {
  ...
  const handleDelete = (id: string) => {
    ...
    updateData((prev) => ({ ...prev, categories: prev.categories.filter((c) => c.id !== id), cashbook: prev.cashbook.map((e) => e.categoryId === id ? { ...e, categoryId: undefined, category: e.category || 'Uncategorized' } : e), expenses: prev.expenses.map((e) => e.categoryId === id ? { ...e, categoryId: undefined } : e), ledgerParties: prev.ledgerParties.map((party) => party.categoryId === id ? { ...party, categoryId: undefined } : party), ledgerEntries: prev.ledgerEntries.map((entry) => entry.categoryId === id ? { ...entry, categoryId: undefined } : entry) }), 'Delete Category', 'Finance', `${role} ${currentUser.name} deleted category "${cat.name}".`)
  }
