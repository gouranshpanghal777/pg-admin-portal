...
function TenantLedgerModal({ tenant, data, onClose }: { tenant: Tenant; data: AppData; onClose: () => void }) {
  const room = data.rooms.find((item) => item.id === tenant.roomId)
  const payments = data.payments.filter((item) => item.tenantId === tenant.id).sort((a, b) => b.date.localeCompare(a.date))
  const obligations = data.obligations.filter((item) => item.tenantId === tenant.id).sort((a, b) => b.period.localeCompare(a.period))
  const security = data.securityLedger.filter((item) => item.tenantId === tenant.id).sort((a, b) => b.date.localeCompare(a.date))
  ...
  return <Modal title={`${tenant.name} - Tenant Ledger`} onClose={onClose}>
    <div className="grid gap-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><div className="rounded-md bg-emerald-50 p-3"><p className="text-xs text-emerald-700">Total paid</p><b>{money(totalPaid)}</b></div><div className="rounded-md bg-blue-50 p-3"><p className="text-xs text-blue-700">Rent paid</p><b>{money(paymentTotalFor('Rent'))}</b></div><div className="rounded-md bg-blue-50 p-3"><p className="text-xs text-blue-700">Security paid</p><b>{money(paymentTotalFor('Security Deposit'))}</b></div><div className="rounded-md bg-blue-50 p-3"><p className="text-xs text-blue-700">Other paid</p><b>{money(paymentTotalFor('Electricity') + paymentTotalFor('Other'))}</b></div></div>
      <section><h3 className="mb-2 font-bold">Monthly Rent & Rate History</h3><DataTable headers={['Rent month', 'Rent agreed', 'Received', 'Advance used', 'Balance', 'Due date', 'Status']}>{obligations.filter((item) => item.paymentType === 'Rent').map((item) => { const received = Math.max(item.received, payments.filter((payment) => payment.paymentType === 'Rent' && payment.month === item.period).reduce((sum, payment) => sum + payment.amount, 0)); const balance = Math.max(0, item.agreed - received - item.advanceApplied); return <tr key={item.id} className="border-t border-slate-100"><td className="p-3">{formatMonth(item.period)}</td><td className="p-3 font-semibold">{money(item.agreed)}</td><td className="p-3 text-emerald-700">{money(received)}</td><td className="p-3">{money(item.advanceApplied)}</td><td className="p-3 text-rose-700">{money(balance)}</td><td className="p-3">{formatDate(item.dueDate)}</td><td className="p-3"><Badge tone={balance === 0 ? 'green' : received > 0 ? 'orange' : 'red'}>{balance === 0 ? 'Paid' : received > 0 ? 'Partial' : item.status}</Badge></td></tr> })}{!obligations.some((item) => item.paymentType === 'Rent') && <tr><td className="p-3 text-slate-500" colSpan={7}>No monthly rent ledger records yet.</td></tr>}</DataTable></section>
