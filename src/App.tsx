...
function MoveTenantModal({ tenant, rooms, tenants, onClose, onSubmit }: { tenant: Tenant; rooms: Room[]; tenants: Tenant[]; onClose: () => void; onSubmit: (roomId: string, note: string) => void }) {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const form = new FormData(event.currentTarget)
      onSubmit(String(form.get('roomId')), String(form.get('note') || ''))
      onClose()
    } finally { savingRef.current = false; setSaving(false) }
  }
  return <Modal title="Move Tenant" onClose={onClose}>{available.length ? <form className="grid gap-4" onSubmit={handleSubmit}><div className="grid gap-3 rounded-md bg-slate-50 p-3 text-sm md:grid-cols-2"><p><b>Tenant:</b> {tenant.name}</p><p><b>Current room:</b> {current?.number}</p></div><Field label="Select new room"><select name="roomId" className={inputClass} required>{available.map((room) => <option key={room.id} value={room.id}>Room {room.number} · {tenants.filter((item) => item.roomId === room.id && item.status !== 'Left').length}/{room.beds} occupied</option>)}</select></Field><Field label="Move date (optional)"><input name="moveDate" className={inputClass} type="date" defaultValue={today} /></Field><Field label="Note/reason (optional)"><textarea name="note" className={inputClass} /></Field><div className="flex justify-end gap-2"><Button tone="soft" onClick={onClose}>Cancel</Button><Button disabled={saving} type="submit">Confirm Move</Button></div></form> : <div><p className="rounded-md bg-orange-50 p-4 text-sm font-semibold text-orange-800">No vacant room available for moving.</p><div className="mt-4 flex justify-end"><Button tone="soft" onClick={onClose}>Close</Button></div></div>}</Modal>
}
