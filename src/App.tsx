    ...
    <div className="min-h-screen bg-[#f7f3ec] text-slate-900">
      ...
      <div className="lg:pl-64">
        ...
        <main className="p-3 lg:p-6" style={{ minHeight: 'calc(100dvh - 3.5rem - env(safe-area-inset-top, 0px))' }}>
          ...
          {page === 'Finance' && <FinancePage scoped={scoped} financeTab={financeTab} setFinanceTab={setFinanceTab} data={data} branch={branch} setModal={setModal} setSelectedTenantId={setSelectedTenantId} setSelectedCashbookId={setSelectedCashbookId} updateData={updateData} role={role} currentUser={currentUser} isAdmin={isAdmin} />}
          ...
          {page === 'Maintenance' && <MaintenancePage data={data} scoped={scoped} filter={ticketFilter} setFilter={setTicketFilter} setModal={setModal} setSelectedTicketId={setSelectedTicketId} canResolve={can('resolve_maintenance')} canCreate={can('create_maintenance')} branch={branch} />}
      ...
      {modal === 'maintenanceQR' && <MaintenanceQRModal branch={branch} onClose={() => setModal('')} />}
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await onSubmit(
        requestId,
        paymentRequestId,
        { name: String(form.get('name')).trim().toUpperCase(), phone: String(form.get('phone')), email: String(form.get('email')), roomId, bedNo: nextBed, joiningDate: String(form.get('joiningDate')), dueDate: String(form.get('dueDate') || form.get('joiningDate')), monthlyRent, security, electricity: String(form.get('electricity')) as 'Included' | 'Fixed', electricityAmount: Number(form.get('electricityAmount') || 0), idProof: String(form.get('idProof') || '').replace(/\s/g, '') },
