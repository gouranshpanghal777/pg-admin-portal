...
import { admitTenant, createStaffAccount, deactivateStaffAccount, deleteCashbookEntryCascade, deleteTenantWithPayments, loadAppData, persistAppData, recordSplitPayment } from './lib/database'
import {
  AlertTriangle,
  Bell,
  Boxes,
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f7f3ec] text-slate-900">
      ...
      {modal === 'admit' && <AdmitTenantModal rooms={scoped.rooms} tenants={scoped.activeTenants} onClose={() => setModal('')} onSubmit={async (requestId, tenant) => {
        setBackendError('')
        try { await admitTenant({ requestId, branchId, ...tenant }); setData(await loadAppData()) }
      ...
      {modal === 'confirmDeleteTenant' && <ConfirmModal title="Permanently delete tenant" message="This permanently deletes the tenant and all of their payment history. Use Vacate instead if the historical record should be kept." onClose={() => setModal('')} onConfirm={async () => {
        setBackendError('')
      ...
      {tenantTab === 'Active' ? <>
        ...
        <DataTable headers={['Tenant name', 'Email/phone', 'Room', 'Room type', 'Monthly rent', 'Rent paid', 'Rent balance', 'Security', 'Security received', 'Security balance', 'Electricity', 'Since', 'Rent Due Date', 'Status', 'Actions']}>
          {filtered.map((tenant) => {
            ...
            const balance = Math.max(0, getTenantDue(tenant) - tenant.paidThisMonth)
            const securityReceived = paymentTotal(scoped.payments, 'Security Deposit', tenant.id, null)
            const securityBalance = Math.max(0, tenant.security - securityReceived)
...
function PaymentsPage({ data, scoped, filter, setFilter, setModal, setSelectedTenantId, canAdd }: { data: AppData; scoped: ReturnType<typeof branchData>; filter: string; setFilter: (value: string) => void; setModal: (value: string) => void; setSelectedTenantId: (id: string) => void; canAdd: boolean }) {
  const tenants = filter === 'Left PG' ? scoped.leftTenants : scoped.activeTenants.filter((tenant) => filter === 'All' || getPaymentStatus(tenant) === filter)
  ...
  const balanceDue = scoped.activeTenants.reduce((sum, tenant) => sum + Math.max(0, tenant.monthlyRent - paymentTotal(scoped.payments, 'Rent', tenant.id)) + Math.max(0, tenant.security - paymentTotal(scoped.payments, 'Security Deposit', tenant.id, null)), 0)
  return <div className="grid gap-4"><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><Metric icon={<IndianRupee />} label="Total Collected" value={money(collected)} /><Metric icon={<ReceiptText />} label="Rent Collected" value={money(rentCollected)} /><Metric icon={<ShieldCheck />} label="Security Received" value={money(securityCollected)} /><Metric icon={<ReceiptText />} label="Balance Due" value={money(balanceDue)} tone="orange" /><Metric icon={<AlertTriangle />} label="Overdue" value={money(scoped.overdue)} tone="red" /></div><Tabs values={['All', 'Paid', 'Pending', 'Overdue', 'Left PG']} value={filter} onChange={setFilter} /><DataTable headers={['Tenant', 'Room', 'Rent Due Date', 'Monthly rent', 'Rent paid', 'Rent balance', 'Security', 'Security received', 'Security balance', 'Electricity', 'Status', 'Actions']}>{tenants.map((tenant) => { const room = data.rooms.find((item) => item.id === tenant.roomId); const rentPaid = paymentTotal(scoped.payments, 'Rent', tenant.id); const rentBalance = Math.max(0, tenant.monthlyRent - rentPaid); const securityReceived = paymentTotal(scoped.payments, 'Security Deposit', tenant.id, null); const securityBalance = Math.max(0, tenant.security - securityReceived); const status = tenant.status === 'Left' ? 'Left PG' : getPaymentStatus({ ...tenant, paidThisMonth: rentPaid }); return <tr key={tenant.id} className="border-t border-slate-100"><td className="p-3 font-semibold">{tenant.name}</td><td className="p-3">{room?.number || 'Archived room'}</td><td className="p-3 font-semibold">{formatDate(tenant.dueDate)}</td><td className="p-3">{money(tenant.monthlyRent)}</td><td className="p-3 text-emerald-700">{money(rentPaid)}</td><td className="p-3 text-rose-700">{money(rentBalance)}</td><td className="p-3">{money(tenant.security)}</td><td className="p-3 text-emerald-700">{money(securityReceived)}</td><td className="p-3">{securityBalance === 0 ? <Badge tone="green">Cleared</Badge> : money(securityBalance)}</td><td className="p-3">{tenant.electricity === 'Fixed' ? money(tenant.electricityAmount) : 'Included'}</td><td className="p-3"><Badge tone={status === 'Paid' ? 'green' : status === 'Overdue' ? 'red' : status === 'Left PG' ? 'slate' : 'orange'}>{status}</Badge></td><td className="p-3">{canAdd && tenant.status !== 'Left' && <Button tone="green" onClick={() => { setSelectedTenantId(tenant.id); setModal('payment') }}><Plus size={15} /> Add received payment</Button>}</td></tr> })}</DataTable></div>
}
...
function FinancePage({ scoped, financeTab, setFinanceTab, data, branch, setModal, setSelectedTenantId, setSelectedCashbookId, isAdmin }: { scoped: ReturnType<typeof branchData>; financeTab: string; setFinanceTab: (value: string) => void; data: AppData; branch: Branch; setModal: (value: string) => void; setSelectedTenantId: (value: string) => void; setSelectedCashbookId: (value: string) => void; isAdmin: boolean }) {
  ...
  return <div className="grid gap-4"><Tabs values={['Cashbook', 'Expenses', 'Bill Creator']} value={financeTab} onChange={setFinanceTab} />{financeTab === 'Cashbook' && <><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><Metric icon={<CircleDollarSign />} label="Opening Balance" value={money(scoped.openingBalance)} /><Metric icon={<IndianRupee />} label="Total IN" value={money(scoped.revenue)} /><Metric icon={<ReceiptText />} label="Total OUT" value={money(scoped.expensesTotal)} tone="red" /><Metric icon={<ReceiptText />} label="Net Movement" value={`${money(Math.abs(scoped.netMovement))} ${scoped.netMovement >= 0 ? 'Cr' : 'Dr'}`} /><Metric icon={<CircleDollarSign />} label="Closing Balance" value={money(scoped.closingBalance)} /></div><p className="text-sm font-semibold text-slate-500">Cashbook summary for {new Date(`${scoped.reportingMonth}-01T00:00:00`).toLocaleDateString('en-IN', { month: 'long', year: 'numeric' })}</p><DataTable headers={['Date', 'Description', 'Credit', 'Debit', 'Running balance', 'Actions']}>{rows.map((entry) => <tr key={entry.id} className="border-t border-slate-100"><td className="p-3">{entry.date}</td><td className="p-3">{entry.description}{entry.source !== 'Manual' && <span className="ml-2 text-xs text-slate-400">Source: {entry.source}</span>}</td><td className="p-3 text-emerald-700">{entry.type === 'Credit' ? money(entry.amount) : '-'}</td><td className="p-3 text-rose-700">{entry.type === 'Debit' ? money(entry.amount) : '-'}</td><td className="p-3 font-bold">{money(entry.running)}</td><td className="p-3">{isAdmin && <div className="flex gap-1"><CompactAction title="Edit entry" disabled={!isAdmin || entry.source !== 'Manual'} onClick={() => { setSelectedCashbookId(entry.id); setModal('editCashbook') }}><Edit3 size={14} /></CompactAction><CompactAction title="Delete entry and linked record" danger onClick={() => { setSelectedCashbookId(entry.id); setModal('confirmDeleteCashbook') }}><Trash2 size={14} /></CompactAction></div>}</td></tr>)}</DataTable><p className="text-xs text-slate-500">Deleting an entry also removes its linked source record and recalculates affected balances.</p></>}{financeTab === 'Expenses' && <><div className="grid gap-4 md:grid-cols-5">{['Grocery', 'Vegetables', 'Gas Cylinder', 'Staff Salary', 'Miscellaneous'].map((category) => <Card key={category}><p className="text-sm text-slate-500">{category}</p><p className="text-xl font-black">{money(scoped.expenses.filter((expense) => expense.category === category).reduce((sum, expense) => sum + expense.amount, 0))}</p></Card>)}</div><Button tone="red" onClick={() => setModal('expense')}><Plus size={16} /> Add Expense</Button><DataTable headers={['Date', 'Category', 'Description', 'Amount', 'Vendor/note']}>{scoped.expenses.map((expense) => <tr key={expense.id} className="border-t border-slate-100"><td className="p-3">{expense.date}</td><td className="p-3">{expense.category}</td><td className="p-3">{expense.description}</td><td className="p-3 text-rose-700">{money(expense.amount)}</td><td className="p-3">{expense.vendor}</td></tr>)}</DataTable></>}{financeTab === 'Bill Creator' && <BillCreator scoped={scoped} data={data} branch={branch} setSelectedTenantId={setSelectedTenantId} />}</div>
}
...
function PaymentModal({ tenants, payments, selectedTenantId, onClose, onSubmit }: { tenants: Tenant[]; payments: Payment[]; selectedTenantId: string; onClose: () => void; onSubmit: (payment: SplitPaymentInput) => Promise<void> }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true); setError('')
    try {
      await onSubmit({ requestId, tenantId, rentAmount, securityAmount, electricityAmount, otherAmount, paymentMode, description: String(form.get('description') || '') })
      onClose()
    } catch (failure) {
...
function ConfirmModal({ title, message, onClose, onConfirm }: { title: string; message: string; onClose: () => void; onConfirm: () => void | Promise<void> }) {
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  const confirm = async () => {
    if (saving) return
  ...
  return <Modal title={title} onClose={onClose}><p className="text-sm text-slate-600">{message}</p>{error && <p className="mt-3 rounded-md bg-rose-50 p-3 text-sm text-rose-700">{error}</p>}<div className="mt-5 flex justify-end gap-2"><Button tone="soft" onClick={onClose}>Cancel</Button><Button tone="red" disabled={saving} onClick={confirm}>{saving ? 'Deleting...' : 'Confirm'}</Button></div></Modal>
}
