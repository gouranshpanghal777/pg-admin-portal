...
import { admitTenant, createStaffAccount, deactivateStaffAccount, deleteCashbookEntryCascade, deleteTenantWithPayments, loadAppData, persistAppData, recordSplitPayment, undoVacateTenant, vacateTenantErp } from './lib/database'
...
function CashbookModal({ entry, categories, onClose, onSubmit }: { entry?: CashbookEntry; categories: string[]; onClose: () => void; onSubmit: (entry: Omit<CashbookEntry, 'id' | 'branchId' | 'source' | 'linkedId'>) => void }) {
...
function PaymentModal({ tenants, payments, obligations, selectedTenantId, onClose, onSubmit }: { tenants: Tenant[]; payments: Payment[]; obligations: PaymentObligation[]; selectedTenantId: string; onClose: () => void; onSubmit: (payment: SplitPaymentInput) => Promise<void> }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true); setError('')
    try {
      await onSubmit({ requestId, tenantId, rentAmount, securityAmount, electricityAmount, otherAmount, paymentDate: String(form.get('paymentDate')), rentPeriod: rentState?.period, paymentMode, description: String(form.get('description') || '') })
      onClose()
    } catch (failure) {
      setError(failure instanceof Error ? failure.message : 'Payment could not be saved.')
    } finally { setSaving(false) }
  }
  return <Modal title="Add Received Payment" onClose={onClose}><form className="grid gap-4" onSubmit={submit}><Field label="Tenant"><select className={inputClass} value={tenantId} onChange={(event) => selectTenant(event.target.value)}>{tenants.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field><div className="grid grid-cols-2 gap-2 rounded-md bg-slate-50 p-3 text-sm"><p>Rent balance ({rentState?.period || '-'})<br /><b>{money(rentBalance)}</b></p><p>Rent due date<br /><b>{formatDate(rentState?.dueDate)}</b></p><p>Security balance<br /><b>{money(securityBalance)}</b></p><p>Security received<br /><b className="text-emerald-700">{money(securityReceived)}</b></p><p>Security total<br /><b>{money(tenant?.security || 0)}</b></p></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Rent received"><input className={inputClass} type="number" min="0" value={rentAmount || ''} placeholder="0" onChange={(event) => setRentAmount(Number(event.target.value))} /></Field><Field label="Security deposit received"><input className={inputClass} type="number" min="0" max={securityBalance || undefined} value={securityAmount || ''} placeholder="0" onChange={(event) => setSecurityAmount(Number(event.target.value))} /></Field><Field label="Electricity received"><input className={inputClass} type="number" min="0" value={electricityAmount || ''} placeholder="0" onChange={(event) => setElectricityAmount(Number(event.target.value))} /></Field><Field label="Other received"><input className={inputClass} type="number" min="0" value={otherAmount || ''} placeholder="0" onChange={(event) => setOtherAmount(Number(event.target.value))} /></Field></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Payment date"><input name="paymentDate" className={inputClass} type="date" defaultValue={today} required /></Field><Field label="Rent month being settled"><input className={inputClass} value={rentState?.period ? formatMonth(rentState.period) : '-'} disabled /></Field><Field label="Payment mode"><select className={inputClass} value={paymentMode} onChange={(event) => setPaymentMode(event.target.value)}><option>Cash</option><option>UPI</option><option>Bank Transfer</option><option>Card</option></select></Field></div><Field label="Description/source optional"><input name="description" className={inputClass} /></Field>{error && <p className="rounded-md bg-rose-50 p-3 text-sm text-rose-700">{error}</p>}<div className="flex justify-end gap-2"><Button tone="soft" onClick={onClose}>Cancel</Button><Button tone="green" type="submit" disabled={saving}>{saving ? 'Saving...' : 'Add Payment'}</Button></div></form></Modal>
}
