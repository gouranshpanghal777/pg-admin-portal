...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f7f3ec] text-slate-900">
      ...
      {modal === 'categoryAccountEntry' && <CategoryAccountEntryModal branchName={branch.name} categories={data.categories.filter((category) => category.branchId === branchId)} parties={scoped.ledgerParties} entries={scoped.ledgerEntries} onClose={closeModal} onSaved={async (message) => { const refreshed = await loadAppData(); dataRef.current = refreshed; setData(refreshed); setSuccessMessage(message) }} />}
