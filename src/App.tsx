...
function CashbookModal({ entry, onClose, onSubmit }: { entry?: CashbookEntry; onClose: () => void; onSubmit: (entry: Omit<CashbookEntry, 'id' | 'branchId' | 'source' | 'linkedId'>) => void }) {
  ...
  return <Modal title={entry ? 'Edit Cashbook Entry' : 'Add Cashbook Entry'} onClose={onClose}><form className="grid gap-4 md:grid-cols-2" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); onSubmit({ type, amount, description, date, category: String(form.get('category')), paymentMode: String(form.get('paymentMode')), reference: String(form.get('reference')), remarks: String(form.get('remarks')) }); onClose() }}><Field label="Transaction type"><select className={inputClass} value={type} onChange={(event) => setType(event.target.value as EntryType)}><option>Credit</option><option>Debit</option></select></Field><Field label="Amount"><input className={inputClass} type="number" min="0.01" step="0.01" required value={amount || ''} onChange={(event) => setAmount(Number(event.target.value))} /></Field><Field label="Description"><input className={inputClass} required value={description} onChange={(event) => setDescription(event.target.value)} /></Field><Field label="Date"><input className={inputClass} type="date" value={date} onChange={(event) => setDate(event.target.value)} /></Field><Field label="Category"><input name="category" className={inputClass} defaultValue={entry?.category || 'Uncategorized'} /></Field><Field label="Cash / Online"><select name="paymentMode" className={inputClass} defaultValue={entry?.paymentMode || 'Cash'}><option>Cash</option><option>Online</option><option>UPI</option><option>Bank Transfer</option><option>Card</option></select></Field><Field label="Reference"><input name="reference" className={inputClass} defaultValue={entry?.reference} /></Field><Field label="Remarks"><input name="remarks" className={inputClass} defaultValue={entry?.remarks} /></Field><div className="flex justify-end gap-2 md:col-span-2"><Button tone="soft" onClick={onClose}>Cancel</Button><Button tone={type === 'Credit' ? 'green' : 'red'} type="submit">{entry ? 'Save Changes' : type === 'Credit' ? 'Add Credit' : 'Add Debit'}</Button></div></form></Modal>
}
