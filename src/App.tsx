...
function TenantLedgerModal({ tenant, data, onClose }: { tenant: Tenant; data: AppData; onClose: () => void }) {
  ...
  const advances = data.advances.filter((item) => item.tenantId === tenant.id).sort((a, b) => b.date.localeCompare(a.date))
  ...
  const knownRentPeriods = new Set([
    ...
    ...payments.filter((item) => item.paymentType === 'Rent').map((item) => item.month),
    ...(importedRentPaidMonths[tenant.name.trim().toUpperCase()] || []),
  ])
