...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f7f3ec] text-slate-900">
      ...
      {modal === 'cashbook' && <CashbookModal branches={data.branches.filter((item) => item.active !== false)} branchId={branchId} categories={Array.from(new Set(scoped.cashbook.map((entry) => entry.category).filter((value): value is string => Boolean(value))))} onClose={() => setModal('')} onSubmit={(entry) => { const interBranch = parseInterBranchReference(entry.reference); updateData((previous) => { const now = new Date().toISOString(); const primary = { id: uid('c'), branchId, source: 'Manual' as const, createdAt: now, ...entry }; const mirror = interBranch?.kind === 'IBS' ? { id: uid('c'), branchId: interBranch.counterpartyBranchId, source: 'Manual' as const, type: 'Debit' as const, amount: interBranch.amount, description: `Inter-branch settlement paid to ${branch.name}`, date: entry.date, category: 'Inter-branch Settlement', paymentMode: entry.paymentMode, reference: `IBS|${branchId}|${interBranch.amount}`, remarks: entry.remarks, createdAt: now } : undefined; return { ...previous, cashbook: [primary, ...(mirror ? [mirror] : []), ...previous.cashbook] } }, entry.type === 'Credit' ? 'credit created' : 'debit created', 'Cashbook', `${role} ${currentUser.name} added cashbook ${entry.type.toLowerCase()} of ${money(entry.amount)}. Description: ${entry.description}.`, { amount: entry.amount, type: entry.type }) }} />}
      ...
      {modal === 'notice' && <NoticeModal onClose={() => setModal('')} onSubmit={(notice) => { const tenant = data.tenants.find((item) => item.id === selectedTenantId)!; updateData((previous) => ({ ...previous, tenants: previous.tenants.map((item) => item.id === selectedTenantId ? { ...item, status: 'Notice', notice } : item) }), 'Issue Tenant Notice', 'Tenants', `${role} ${currentUser.name} issued vacating notice to ${tenant.name} in Room ${data.rooms.find((room) => room.id === tenant.roomId)?.number}. Notice details: ${notice}.`) }} />}
      ...
      {modal === 'expense' && <ExpenseModal onClose={() => setModal('')} onSubmit={(expense) => { const expenseId = uid('e'); const cashbookId = uid('c'); const now = new Date().toISOString(); updateData((previous) => ({ ...previous, expenses: [{ id: expenseId, branchId, cashbookId, ...expense }, ...previous.expenses], cashbook: [{ id: cashbookId, branchId, type: 'Debit', amount: expense.amount, description: expense.description, date: expense.date, source: 'Expense', linkedId: expenseId, category: expense.category, createdAt: now }, ...previous.cashbook] }), 'debit created', 'Finance', `${role} ${currentUser.name} added expense of ${money(expense.amount)} under ${expense.category}. Vendor/Note: ${expense.vendor || '-'}.`) }} />}
...
function FinancePage({ scoped, financeTab, setFinanceTab, data, branch, setModal, setSelectedTenantId, setSelectedCashbookId, isAdmin }: { scoped: ReturnType<typeof branchData>; financeTab: string; setFinanceTab: (value: string) => void; data: AppData; branch: Branch; setModal: (value: string) => void; setSelectedTenantId: (value: string) => void; setSelectedCashbookId: (value: string) => void; isAdmin: boolean }) {
  ...
  const expenseCategories = [...new Set(scoped.expenses.map((e) => e.category).filter(Boolean))].sort()
  const filteredExpenses = expenseCategory === 'All Categories' ? scoped.expenses : scoped.expenses.filter((e) => e.category === expenseCategory)
  const filteredExpenseTotal = filteredExpenses.reduce((s, e) => s + e.amount, 0)
