...
const empty: AppData = { branches: [], users: [], tenants: [], rooms: [], payments: [], cashbook: [], expenses: [], inventory: [], purchases: [], tickets: [], invoices: [], activityLogs: [] }
const num = (value: unknown) => Number(value || 0)

export async function loadAppData(): Promise<AppData> {
  const tables = ['branches', 'profiles', 'staff_members', 'branch_assignments', 'staff_permissions', 'rooms', 'tenants', 'payments', 'cashbook_entries', 'expenses', 'inventory_items', 'inventory_purchases', 'maintenance_tickets', 'invoices', 'activity_logs'] as const
  const results = await Promise.all(tables.map((table) => supabase.from(table).select('*')))
  const failed = results.find((result) => result.error)
  if (failed?.error) throw failed.error
  const [branches, profiles, staff, assignments, permissions, rooms, tenants, payments, cashbook, expenses, inventory, purchases, tickets, invoices, logs] = results.map((result) => result.data || [])
  ...
  return {
    ...
    payments: payments.map((r) => ({ id: r.id, branchId: r.branch_id, tenantId: r.tenant_id, amount: num(r.amount), date: r.payment_date, month: r.month, status: r.status, invoiceId: r.invoice_id })),
...
const rows = {
  ...
  payments: (r: any, userId: string) => ({ id: r.id, branch_id: r.branchId, tenant_id: r.tenantId, amount: r.amount, payment_date: r.date, month: r.month, status: r.status, invoice_id: r.invoiceId || null, created_by: userId }),
