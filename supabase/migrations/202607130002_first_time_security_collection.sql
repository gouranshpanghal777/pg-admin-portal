-- Allow first-time security deposit collection.
--
-- PROBLEM: record_split_payment rejects ANY security payment where
-- p_security_amount > security_balance.  Tenants admitted with
-- security=0 have security_balance=0, so even a legitimate first-time
-- security collection (which should SET the security total) is blocked.
--
-- FIX:
-- 1. Skip the balance check when security=0 (first-time).
-- 2. In the tenant update, auto-set security = payment_amount when
--    security=0 and security_amount>0.
...
begin
  ...
  -- Security validation:
  --  * First-time (security=0): allow any positive amount — the tenant update
  --    below will set security = payment_amount, making balance = 0.
  --  * Existing  (security>0): must not exceed remaining balance.
  if coalesce(p_security_amount, 0) > 0
     and v_tenant.security > 0
     and p_security_amount > v_tenant.security_balance then
  ...
  if coalesce(p_security_amount, 0) > 0 then
    ...
    values (p_branch_id, p_tenant_id, p_security_amount, p_payment_date, v_month,
      case when v_tenant.security = 0 or v_tenant.security_received + p_security_amount >= v_tenant.security then 'Received' else 'Partial' end,
  ...
  -- Atomic tenant update:
  --  * paid_this_month  += rent
  --  * security_received += security
  --  * security          = payment_amount  (first-time only, when security was 0)
  ...
  set paid_this_month = paid_this_month + coalesce(p_rent_amount, 0),
      ...
      security = case
        when v_tenant.security = 0 and coalesce(p_security_amount, 0) > 0
          then p_security_amount
        else security
        end,
